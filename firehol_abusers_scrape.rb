...
url = URI("https://iplists.firehol.org/files/firehol_abusers_30d.netset")
...
headers = ["ip"]
CSV.open("data_collected/abusers/firehol_abusers_list.csv", "w", :col_sep => "\t| ", :headers => true) do |csv|
    csv << headers
    list.each do |row| 
        temp = []
        temp.append(row)
